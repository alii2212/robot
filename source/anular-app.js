! function (window, document, undefined) {
    ...
    function minErr(module, ErrorConstructor) {
        return ErrorConstructor = ErrorConstructor || Error,
            function () {
                ...
                }), message = message + "\nhttp://errors.angularjs.org/1.3.5/" + (module ? module + "/" : "") + code, i = 2; i < arguments.length; i++) message = message + (2 == i ? "?" : "&") + "p" + (i - 2) + "=" + encodeURIComponent(toDebugString(arguments[i]));
    ...
    function getNgAttribute(element, ngAttr) {
        ...
        for (element = jqLite(element), i = 0; ii > i; ++i)
            if (attr = ngAttrPrefixes[i] + ngAttr, isString(attr = element.attr(attr))) return attr;
        return null
    }
        ...
        this.debugInfoEnabled = function (enabled) {
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
                ...
                function getControllers(directiveName, require, $element, elementControllers) {
                    var value, match, retrievalMethod = "data",
                        optional = !1,
        ...
        this.digestTtl = function (value) {
            ...
            Scope.prototype = {
                ...
                $watch: function (watchExp, listener, objectEquality) {
                    ...
                    return lastDirtyWatch = null, isFunction(listener) || (watcher.fn = noop), array || (array = scope.$$watchers = []), array.unshift(watcher),
                        function () {
    ...
    function $TimeoutProvider() {
        this.$get = ["$rootScope", "$browser", "$q", "$$q", "$exceptionHandler", function ($rootScope, $browser, $q, $$q, $exceptionHandler) {
            function timeout(fn, delay, invokeApply) {
                var timeoutId, skipApply = isDefined(invokeApply) && !invokeApply,
                    deferred = (skipApply ? $$q : $q).defer(),
                    promise = deferred.promise;
        ...
        return function (array, expression, comparator) {
            ...
            predicates.check = function (value, index) {
                for (var j = 0; j < predicates.length; j++)
                    if (!predicates[j](value, index)) return !1;
                return !0
    ...
    function FormController(element, attrs, $scope, $animate, $interpolate) {
        ...
        form.$error = {}, form.$$success = {}, form.$pending = undefined, form.$name = $interpolate(attrs.name || attrs.ngForm || "")($scope), form.$dirty = !1, form.$pristine = !0, form.$valid = !0, form.$invalid = !1, form.$submitted = !1, parentForm.$addControl(form), form.$rollbackViewValue = function () {
            ...
            set: function (object, property, control) {
                ...
                if (list) {
                    var index = list.indexOf(control); - 1 === index && list.push(control)
    ...
    Parser.ZERO = extend(function () {
        ...
        functionCall: function (fnGetter, contextGetter) {
            ...
            return function (scope, locals) {
                ...
                if (args)
                    for (var i = argsFn.length; i--;) args[i] = ensureSafeObject(argsFn[i](scope, locals), expressionText);
            ...
            addSetValidityMethod({
                ...
                if (!isNumber(ctrl.$modelValue) || !isNaN(ctrl.$modelValue)) {
                    var viewValue = ctrl.$$lastCommittedViewValue,
                        ...
                        parserValid = ctrl.$error[parserName] ? !1 : undefined,
                ...
                function processParseErrors(parseValid) {
                    ...
                    if (parseValid === undefined) setValidity(errorKey, null);
                    else if (setValidity(errorKey, parseValid), !parseValid) return forEach(ctrl.$validators, function (v, name) {
                ...
                var viewValue = ctrl.$$lastCommittedViewValue,
                    ...
                    parserValid = isUndefined(modelValue) ? undefined : !0;
                if (parserValid)
                    ...
                    function setupAsOptions(scope, selectElement, ctrl) {
                        ...
                        function createIsSelectedFn(viewValue) {
                            ...
                            if (multiple)
                                ...
                                } else selectedSet = new HashMap(viewValue);
                            else trackFn && (viewValue = callExpression(trackFn, null, viewValue));
                            return function (key, value) {
                                var compareValueFn;
    ...
    angular.module("ngAnimate", ["ng"]).directive("ngAnimateChildren", function () {
        ...
        $provide.decorator("$animate", ["$delegate", "$$q", "$injector", "$sniffer", "$rootElement", "$$asyncCallback", "$rootScope", "$document", "$templateRequest", function ($delegate, $$q, $injector, $sniffer, $rootElement, $$asyncCallback, $rootScope, $document, $templateRequest) {
            ...
            function animateSetup(animationEvent, element, className, styles) {
                ...
                if (structural && 0 === transitionDuration && 0 === animationDuration) return element.removeClass(className), !1;
            ...
            function animateRun(animationEvent, element, className, activeAnimationComplete, styles) {
                ...
                if (0 === maxDuration) return element.removeClass(activeClassName), animateClose(element, className), void activeAnimationComplete();
...
angular.module("foundation.common.animations", ["ngAnimate"]), angular.module("foundation.common.animations").animation(".ui-animation", ["$state", "$rootScope", function ($state, $rootScope) {
    var events = ["webkitAnimationEnd", "mozAnimationEnd", "MSAnimationEnd", "oanimationend", "animationend", "webkitTransitionEnd", "otransitionend", "transitionend"],
...
angular.module("foundation.common.directives", []), angular.module("foundation.common.directives").directive("zfClose", ["FoundationApi", function (foundationApi) {
    return {
        ...
        link: function (scope, element) {
            for (var parentElement = !1, tempElement = element.parent(); parentElement === !1;) "BODY" == tempElement[0].nodeName && (parentElement = ""), "undefined" != typeof tempElement.attr("zf-closable") && tempElement.attr("zf-closable") !== !1 && (parentElement = tempElement), tempElement = tempElement.parent();
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            var isActive = !1,
                animationIn = attrs.animationIn,
                animationOut = attrs.animationOut,
            ...
            foundationApi.subscribe(attrs.id, function (msg) {
                if ("show" === msg || "open" === msg) animate(animationIn, !0);
                else if ("hide" === msg || "close" === msg) animate(animationOut, !1);
                else if ("toggle" === msg) {
                    var newState = !isActive,
                        newAnimation = newState ? animationIn : animationOut;
                    setTimeout(function () {
...
angular.module("foundation.common.services", []), angular.module("foundation.common.services").service("FoundationApi", function () {
    ...
    return {
        ...
        generateUuid: function () {
            ...
            do {
                uuid += "zf-uuid-";
                for (var i = 0; 15 > i; i++) uuid += Math.floor(16 * Math.random()).toString(16)
        ...
        animate: function (element, futureState, animationIn, animationOut) {
            var initClasses = ["ng-enter", "ng-leave"],
                activeClasses = ["ng-enter-active", "ng-leave-active"],
                activeGenericClass = "is-active",
                events = ["webkitAnimationEnd", "mozAnimationEnd", "MSAnimationEnd", "oanimationend", "animationend", "webkitTransitionEnd", "otransitionend", "transitionend"],
                reflow = function () {
                    return element[0].offsetWidth
                },
                reset = function () {
                ...
                animate = function (animationClass, activation) {
                    var initClass = activation ? initClasses[0] : initClasses[1],
                        activeClass = activation ? activeClasses[0] : activeClasses[1];
...
angular.module("foundation.init", ["foundation.common.services"]), angular.module("foundation.init").factory("FoundationInit", ["helpers", "FoundationApi", "Utils", function (helpers, foundationApi, u) {
    return {
        init: function () {
            ...
            for (var key in mediaQueries) mediaQueries[key] = "only screen and (min-width: " + mediaQueries[key].replace("rem", "em") + ")";
    ...
    return {
        headerHelper: function (classArray) {
            for (var i = classArray.length, head = angular.element(document.querySelectorAll("head")); i--;) head.append('<meta class="' + classArray[i] + '" />')
        },
        ...
        processStyleToJSON: function (str) {
            ...
            try {
                ...
                return JSON.parse(clean.slice(1, -1))
            }
        }
    }
    ...
    this.registerDynamicRoutes = function (routes) {
        ...
        angular.forEach(dynamicRoutes, function (page) {
            if (page.hasComposed === !0) angular.isDefined(complexViews[page.parent]) || (complexViews[page.parent] = {
                children: {}
            }), complexViews[page.parent].children[page.name] = page;
            else if (page.composed === !0) angular.isDefined(complexViews[page.name]) || (complexViews[page.name] = {
                children: {}
...
angular.module("foundation.accordion", []), angular.module("foundation.accordion").controller("ZfAccordionController", ["$scope", function ($scope) {
    var controller = this,
        ...
        autoOpen = (controller.multiOpen = !1, controller.autoOpen = $scope.autoOpen = $scope.autoOpen || "true");
    controller.select = function (selectSection) {
        ...
        sections.push({
            scope: sectionScope
        }), 1 === sections.length && "true" === autoOpen && (sections[0].active = !0, sections[0].scope.active = !0)
    ...
    return {
        ...
        link: function (scope, element, attrs, controller) {
            controller.multiOpen = "true" === scope.multiOpen ? !0 : !1
        }
    }
...
angular.module("foundation.interchange", ["foundation.common.services"]), angular.module("foundation.interchange").directive("zfInterchange", ["FoundationApi", "$compile", "$http", "$templateCache", "$animate", function (foundationApi, $compile, $http, $templateCache) {
    ...
    return {
        ...
        template: "<div></div>",
        link: function (scope, element, attrs, ctrl, transclude) {
            var childScope, current, scenarios, innerTemplates, named_queries = {
                "default": "only screen",
                landscape: "only screen and (orientation: landscape)",
                portrait: "only screen and (orientation: portrait)",
                retina: "only screen and (-webkit-min-device-pixel-ratio: 2),only screen and (min--moz-device-pixel-ratio: 2),only screen and (-o-min-device-pixel-ratio: 2/1),only screen and (min-device-pixel-ratio: 2),only screen and (min-resolution: 192dpi),only screen and (min-resolution: 2dppx)"
            },
                globalQueries = foundationApi.getSettings().media_queries;
            ...
            var matched = function () {
                ...
                if (count > 0)
                    for (; count--;) {
                        ...
                        mq = matchMedia(named_queries[rule] ? named_queries[rule] : rule), mq.matches && matches.push({
                            ind: count
                        })
                    }
                return matches
                ...
                collectInformation = function (parentElement) {
                    ...
                    angular.forEach(elements, function (el) {
                        ...
                        elem.attr("src") && elem.attr("src").match(/.html$/) ? scenarios[i] = {
                            media: elem.attr("media"),
                            src: elem.attr("src")
                        } : (innerTemplates[i] = elem, scenarios[i] = {
                            media: elem.attr("media"),
                            templ: i
                        }), i++
                    })
            ...
            foundationApi.subscribe("resize", function () {
                transclude(function (clone, newScope) {
                    scenarios && innerTemplates || collectInformation(clone);
                    var ruleMatches = matched(),
                        scenario = 0 === ruleMatches.length ? null : scenarios[ruleMatches[0].ind];
                    if (scenario && checkScenario(scenario)) {
                        if (childScope && (childScope.$destroy(), childScope = null), "undefined" != typeof scenario.templ) {
                            childScope = newScope;
                            var tmp = document.createElement("div");
                            tmp.appendChild(innerTemplates[scenario.templ][0]), element.html(tmp.innerHTML), $compile(element.contents())(childScope), current = scenario
                        } else {
            ...
            return {
                ...
                post: function (scope, element, attrs) {
                    ...
                    var animationIn = attrs.animationIn || "fadeIn",
                        animationOut = attrs.animationOut || "fadeOut",
                        overlayIn = "fadeIn",
                        overlayOut = "fadeOut";
                    foundationApi.subscribe(attrs.id, function (msg) {
...
angular.module("foundation.notification", ["foundation.common.services"]), angular.module("foundation.notification").controller("ZfNotificationController", ["$scope", "FoundationApi", function ($scope, foundationApi) {
    var controller = this,
        notifications = controller.notifications = $scope.notifications = [];
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            element.on("click", function (e) {
                e.preventDefault(), foundationApi.publish(attrs.zfNotify, {
                    title: scope.title,
        ...
        link: function (scope, element, attrs) {
            ...
            var attachment = scope.pinTo || "top center",
                ...
                tetherElement = function (target) {
                    tetherInit || (scope.target = document.getElementById(scope.target ? scope.target : target), tether = new Tether({
                        element: element[0],
                        target: scope.target,
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            var target = attrs.zfPopupToggle,
                id = attrs.id || foundationApi.generateUuid();
...
angular.module("foundation.tabs", ["foundation.common.services"]), angular.module("foundation.tabs").controller("ZfTabsController", ["$scope", "FoundationApi", function ($scope, foundationApi) {
    var controller = this,
        tabs = controller.tabs = $scope.tabs = [],
        id = "";
    controller.select = function (selectTab) {
        ...
        tabs.push({
            scope: tabScope,
            active: !1,
            parentContent: controller.id
        }), 1 === tabs.length && (tabs[0].active = !0, tabScope.active = !0)
